...
module.exports = class Kaspad extends Daemon {
		...
		return new Promise(async (resolve,reject) => {
			...
			if (this.task.conf['upnpEnabled'] && args.listen?.indexOf(":") != -1) {
				...
				if (externalip) {
					...
					const publicPort = await this.forwardPort(nodePort, nodePort, true);
					if (publicPort && !args['maxinpeers']) {
	...
	forwardPort(privatePort, publicPort, retry) {
		return this.upnpClient.createMapping({
			...
			this.writeToSink(`UPNP: Public port ${publicPort} successfully mapped to local port ${privatePort}\n`.green);
			this.scheduleRenewal(privatePort, publicPort);
			return publicPort;
		}).catch((err) => {
			if (retry) {
	...
	scheduleRenewal(privatePort, publicPort) {
		...
		this.upnpRenewalInterval = setInterval(() => {
			...
			this.forwardPort(privatePort, publicPort);
