...
module.exports = class Kaspad extends Daemon {
	...
	start() {
		...
		return new Promise(async (resolve,reject) => {
			...
			if(this.task.conf['upnpEnabled'] !== false)
				this.task.conf['upnpEnabled'] = false; // Set to true to enable if not explicitly disabled
			...
			let externalip=0
			if (this.task.conf['upnpEnabled'] && args.listen?.indexOf(":") != -1) {
				const nodePort = args.listen.split(':')[1];
				externalip = await this.forwardPort(nodePort, nodePort, true);
			}
			if (externalip) {
	...
	forwardPort(privatePort, publicPort, retry) {
   		return this.upnpClient.getGateway().then((device) => {
			const address = device.address;
			console.debug(`UPNP: Found gateway, local address = ${address}`);
			return this.upnpClient.getPublicIp().then((publicIp) => {
				console.debug(`UPNP: Found public IP: ${publicIp}`);
				return this.upnpClient.createMapping({
					...
					console.info(`UPNP: Public port ${publicPort} successfully mapped to ${address}:${privatePort}`);
					setTimeout(() => {this.forwardPort(privatePort, publicPort)}, 1000 * this.upnpTtl * 4 / 3);
					return publicIp;
				}).catch((err) => {
					if (retry) {
						const alternativePort = Math.floor(Math.random() * 45000) + 20000;
						console.warn(`UPNP: Mapping of public port ${publicPort} failed, trying alternative port ${alternativePort}.`);
						return this.forwardPort(privatePort, alternativePort);
					} else {
						console.warn(`UPNP: Mapping of public port ${publicPort} failed.`);
					}
				})
			}).catch((err) => {
				console.info(`UPNP: Couldn't find public IP: ${err.message}`);
			})
		}).catch((err) => {
			console.info(`UPNP: Couldn't find gateway: ${err.message}`);
		})
	}
