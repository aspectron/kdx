...
// try {
// //	const { html, render } = import('lit-html');
// //	const { repeat  } = import('lit-html/directives/repeat.js');
// } catch(ex) {
// 	console.log("+----------------------- FAILURE")
// 	console.log(ex);
// }
	...
	async renderModuleInfo(html) {
		...
		if(this.rpcInfo) {
			...
			const data =  {
				...
				"Blocks" : (i.blocks).toCS(),
				"Difficulty" : i.difficulty,
				"Relay Fee" : i.relayFee,
				//"Errors" : i.errors,
				"Median Time" : { time : this.getTS(bdi.medianTime*1000) },
				'line-break' : 'line-break',
				//"Pruned" : bdi.pruned?'Yes':'No',
				"Mempool Size" : (mpi.size).toCS(),
				"Mempool" : { value : (mpi.bytes).toFileSize(), suffix : true },
				"Network Rx" : { value : (net.totalBytesRecv).toFileSize(), suffix : true },
				"Network Tx" : { value : (net.totalBytesSent).toFileSize(), suffix : true },
				"Network Time" : { time : this.getTS(net.timeMillis) }
			};

			if(i.errors)
				data.Errors = JSON.stringify(i.errors);

			let table = html`<div class="kaspad">
				${
					Object.entries(data).map(([k,v]) => {
						...
						// 	suffix = parts.shift();
						// }
						return html`<flow-data-field has-colon style="${css}" prefix="${prefix}" suffix="${suffix}" title="${k}">${v}</flow-data-field>`;
					}).filter(v=>v)
				}
			</div>`;
