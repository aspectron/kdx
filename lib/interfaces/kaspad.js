...
module.exports = class Kaspad extends Daemon {
	...
	//			debuglevel : 'debug',
				...
				rpcuser : 'user',
				rpcpass : 'pass',
				rpccert : path.join(core.dataFolder,'rpc.cert'),
				rpckey : path.join(core.dataFolder,'rpc.key'),
				nobanning : 1,
				nodnsseed : 1,
	...
	//			listen : '0.0.0.0:'+(env.KASPAD_P2P_BASE_PORT+this.task.seq),
			...
			let addresses = {
				'regnet' : 'kaspareg:qrgufg4qzfqacfzrf6xvx46aqedr5nke3qv6a74fex',
				'devnet' : 'kaspatest:przhjdpv93xfygpqtckdc2zkzuzqeyj2pgg3hlfnac',
				'simnet' : 'kaspasim:qrgufg4qzfqacfzrf6xvx46aqedr5nke3qv6a74fex',
				'kaspa' : 'kaspa:qrgufg4qzfqacfzrf6xvx46aqedr5nke3qv6a74fex'
			};
			...
			if(!args.miningaddr)
				args.miningaddr = 'kaspa:qrgufg4qzfqacfzrf6xvx46aqedr5nke3qv6a74fex';
			...
			// dpc(2500, () => {
			// 	this.rpc.connect(`wss://user:pass@localhost:${(this.globals.kaspad.ports.rpc+this.task.seq)}/ws`);
			// })

			// dpc(2500, async () => {
			// 	this.observer = this.core.kstats.createObserver({
			// 		name : this.task.name,
			// 		address : `wss://user:pass@localhost:${(this.globals.kaspad.ports.rpc+this.task.seq)}/ws`,
			// 	});
			// 	await this.observer.start();
			// 	//this.rpc.connect(`wss://user:pass@localhost:${(this.globals.kaspad.ports.rpc+this.task.seq)}/ws`);

			// 	this.rpc = this.observer.rpc;
