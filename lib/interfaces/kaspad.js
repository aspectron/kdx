			...
			${
				this.data.map((o) => {
					...
					if(!o.graph)
						...
						return html`<flow-data-badge-graph has-colon style="${css}" prefix="${prefix}" suffix="${suffix}" title="${i18n.t(name)}" align="${align}" sampler="${sampler}">${value}</flow-data-badge-graph>`;
