			...
			let table = html`
				${
					this.data.map((o) => {
						...
						// 	suffix = parts.shift();
						// }
						return html`<flow-data-field-graph has-colon style="${css}" prefix="${prefix}" suffix="${suffix}" title="${name}" align="${align}" sampler="${sampler}">${value}</flow-data-field-graph>`;
					}).filter(v=>v)
				}
			`;
		...
		return new Promise(async (resolve, reject) => {
	        ...
	        try {
				...
				const sources =  {
					...
					"Blocks" : { sample : i.blocks, value : (i.blocks).toCS() },
					"Difficulty" : { sample : i.difficulty },
					"Relay Fee" : { sample : i.relayFee },
					//"Errors" : i.errors,
					"Median Time" : { time : this.getTS(bdi.medianTime*1000), sample : bdi.medianTime },
					//'line-break' : 'line-break',
					//"Pruned" : bdi.pruned?'Yes':'No',
					"Mempool Size" : { sample : mpi.size, value : (mpi.size).toCS() },
					"Mempool" : { sample : mpi.bytes, value : (mpi.bytes).toFileSize(), suffix : true },
					"Network Rx" : { sample : net.totalBytesRecv, value : (net.totalBytesRecv).toFileSize(), suffix : true, css : 'min-width:160px' },
					"Network Tx" : { sample : net.totalBytesSent, value : (net.totalBytesSent).toFileSize(), suffix : true, css : 'min-width:160px' },
					"Network Time" : { sample: net.timeMillis, time : this.getTS(net.timeMillis) }
				};
