				...
				${
					Object.entries(data).map(([k,v]) => {
						...
						if(v.time) {
							...
							css += 'min-width:210px;'
							align = 'left';
						} else if(/^blocks/i.test(k)) {
							css += 'min-width:120px;'
						} else if(/^difficulty/i.test(k)) {
							css += 'min-width:120px;'
						}
						...
						// 	suffix = parts.shift();
						// }
						return html`<flow-data-field has-colon style="${css}" prefix="${prefix}" suffix="${suffix}" title="${k}" align="${align}">${v}</flow-data-field>`;
					}).filter(v=>v)
				}
			`;
