...
const WS_JSON_RPC = require('../ws-json-rpc');
const KaspaRPC = require('../rpc');
...
// 			}
			...
			*/
			...
			if(process.env.KASPA_JSON_RPC) {
				// kaspa pre-7.x
				this.rpc = new WS_JSON_RPC(this.task.id);
				this.rpc.on('ws:open', () => {
					if(this.isInit) {
						this.isInit = false;
				...
				if(addr && !['0.0.0.0','127.0.0.1','localhost'].includes(addr))
					iface = addr;
				this.rpc.setAddress(`${iface}:${port}`);
				this.rpc.connect();
			}
	...
	async getStats() {
		if(process.env.KASPA_JSON_RPC)
			return this.getStats_WS_JSON_RPC();
		else
			return this.getStats_gRPC();
	}
		...
		return new Promise(async (resolve, reject) => {
	        ...
	        try {
				...
				const sources =  {
					...
					//"Errors" : i.errors,
					"Median Time" : { time : this.getTS(pastMedianTime*1000), sample : pastMedianTime, graph : true },
...
//					"Network Time" : { sample: net.timeMillis, time : this.getTS(net.timeMillis), graph : true }
	...
	async getStats_WS_JSON_RPC(post) {

		return new Promise(async (resolve, reject) => {
			const data = { }
	        try {
	            for(let method of ['getInfo','getBlockDagInfo','getMempoolInfo','getConnectionCount'])
