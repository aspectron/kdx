			...
			let table = html`
				${
					this.data.map((o) => {
						...
						// }
						if(!o.graph)
							return html`<flow-data-field has-colon style="${css}" prefix="${prefix}" suffix="${suffix}" title="${name}" align="${align}">${value}</flow-data-field>`;
						else
		...
		return new Promise(async (resolve, reject) => {
	        ...
	        try {
				...
				const sources =  {
					...
					"Mempool Size" : { sample : mpi.size, value : (mpi.size).toCS(), graph : true },
					"Mempool" : { sample : mpi.bytes, value : (mpi.bytes).toFileSize(), suffix : true, graph : true },
					"Network Rx" : { sample : net.totalBytesRecv, value : (net.totalBytesRecv).toFileSize(), suffix : true, css : 'min-width:160px', graph : true },
					"Network Tx" : { sample : net.totalBytesSent, value : (net.totalBytesSent).toFileSize(), suffix : true, css : 'min-width:160px', graph : true },
