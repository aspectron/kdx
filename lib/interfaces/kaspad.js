...
//				const sync = (blockCount/headerCount)*100;
				...
				if(this.syncData) {
					const ts_ = new Date();
					const ts = ts_.getTime();//+(ts_.getTimezoneOffset()*60*1000);
					// let ts = Date.now();
					const total = ts - this.syncData.medianTimeStart.timestamp;
				...
				const sources =  {
					...
					"Median Delta" : { sample : syncSampleT, value : syncValueT, graph : true },
...
//console.log("KDX INSTANCE:",this);
				...
				if(this.task.id == 'kd0') {
					...
					this.manager.emit(`sync-status`, { sync: syncSample, headerCount, blockCount, id : this.task.id });
				}
