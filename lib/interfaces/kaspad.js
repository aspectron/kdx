...
module.exports = class Kaspad extends Daemon {
	...
	start() {
		...
		return new Promise(async (resolve,reject) => {
			...
			let externalip=0
			await setTimeout(500); // Ensure "sink" has enough time to get ready to receive messages before proceeding
	...
	forwardPort(privatePort, publicPort, retry) {
   		return this.upnpClient.getGateway().then((device) => {
			...
			this.writeToSink(`UPNP: Found gateway, local address = ${address}\n`);
			return this.upnpClient.getPublicIp().then((publicIp) => {
				this.writeToSink(`UPNP: Found public IP: ${publicIp}\n`);
				return this.upnpClient.createMapping({
					...
					this.writeToSink(`UPNP: Public port ${publicPort} successfully mapped to ${address}:${privatePort}\n`.green);
					...
					if (retry) {
						...
						this.writeToSink(`UPNP: Mapping of public port ${publicPort} failed, trying alternative port ${alternativePort}.\n`.yellow);
						return this.forwardPort(privatePort, alternativePort);
					} else {
						this.writeToSink(`UPNP: Mapping of public port ${publicPort} failed: ${err.message}\n`.red);
					}
				})
			}).catch((err) => {
				this.writeToSink(`UPNP: Couldn't find public IP: ${err.message}\n`.red);
			})
		}).catch((err) => {
			this.writeToSink(`UPNP: Couldn't find gateway: ${err.message}\n`).yellow;
		})
	}
