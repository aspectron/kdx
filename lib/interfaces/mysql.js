...
module.exports = class mySQL extends Daemon {
	...
	start() {
		...
		return new Promise(async (resolve,reject) => {
			...
			this.logFile = path.join(this.folder,'logs',`mysql-${this.task.name}.log`);
	...
	//			`--console`
			...
			if(os.platform() != 'win32') {
				...
				args.push(`--pid-file=/var/run/mysqld/${this.task.name}-${this.uid}.pid`);
				args.push(`--socket=/var/run/mysqld/${this.task.name}-${this.uid}.sock`);
			}
			...
			if(!fs.existsSync(path.join(this.dataFolder,'mysql'))) {
				...
				dpc(MYSQL_STARTUP_DELAY, () => {
					...
					const mysqlInitArgs = [ ];
					// if(os.platform() != 'win32')
					mysqlInitArgs.push(`--socket=/var/run/mysqld/${this.task.name}-${this.uid}.sock`);
					...
					execFile(this.binary.mysql,mysqlInitArgs,{
						...
						this.log("MySQL configuration successful!".color(48));
