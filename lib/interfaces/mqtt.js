		...
		return new Promise(async (resolve,reject) => {
            ...
            let mqttBinary = {
                ...
                'darwin': [
                    ...
                    path.join(this.getBinaryFolder(),'/brew/mosquitto/1.6.11/sbin/mosquitto'),
                    path.join(this.getBinaryFolder(),'/brew/mosquitto/1.6.10/sbin/mosquitto'),
                    path.join(this.getBinaryFolder(),'/brew/mosquitto/1.6.9/sbin/mosquitto'),
                    ...
                    '/opt/local/sbin/mosquitto'
                ],
                'linux': '/usr/sbin/mosquitto'
            }[os.platform()];

            if(os.platform() == 'darwin') {
                const brew = path.join(this.getBinaryFolder(),'brew');
                if(fs.existsSync(brew)) {
                    let list = fs.readdirSync(brew);
                    let targets = [];
                    while(list.length) {
                        let f = list.shift();
                        let versions = fs.readdirSync(path.join(brew,f));
                        while(versions.length) {
                            let v = versions.shift();
                            let target = path.join(brew,f,v,'lib');
                            if(fs.existsSync(target))
                                targets.push(target);
                        }
                    }
                    let p = (process.env.DYLD_LIBRARY_PATH||'').split(':');
                    p = p.concat(targets);
                    p = [...new Set(p)];
                    process.env.DYLD_LIBRARY_PATH = p.join(':');
                }
            }
