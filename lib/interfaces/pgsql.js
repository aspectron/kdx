...
console.log("CONFIG:".brightRed,this.task.conf);
			...
			if(!fs.existsSync(path.join(this.dataFolder,'pg_version'))) {
				...
				// console.log("Sleeping while pgSQL is starting up...".color(48));
				// await this.sleep(3000);
				// console.log("Sleep done...  pgSQL should be running by now...".color(48));
				...
				dpc(PGSQL_STARTUP_DELAY, async () => {
					...
					/*,async (error, stdout, stderr)=>{
						...
						// 		this.log(`Connecting to instantiated pgSQL Db - port: ${port}`.brightYellow);
						...
						// 			if(err) {
						// 				this.log(err);
						// 				this.log("FATAL - pgSQL STARTUP SEQUENCE! [2]".brightRed);
						// 				return;// resolve();
						// 			}

						// 			this.log("pgSQL connection SUCCESSFUL!".brightYellow);
						// 			db.end(()=>{
						// 				this.log("pgSQL client disconnecting.".brightYellow);
						// 			});
						// 		});
						// 	});
						// }
