...
//				args.miningaddr = 'kaspa:qqg767gkznmsuah5xeer9f72lwqu642nqvlmujtpmk';
	...
	async renderModuleInfo({ html }) {
		...
		let rate = html`
			<flow-data-badge-graph style="min-width:128px;" sampler="${this.task.key}-block-rate" suffix=" / SEC" title="BLOCK RATE" align="right">${this.blockrate.toFixed(2)}</flow-data-badge-graph>
			<flow-data-badge-graph style="min-width:128px;" sampler="${this.task.key}-block-rate-avg" suffix=" / MIN" title="BLOCKS RATE (AVG)" align="right">${this.blockrate1m.toFixed(2)}</flow-data-badge-graph>
			<flow-data-badge-graph style="min-width:160px;" sampler="${this.task.key}-hash-rate" suffix="${this.hashrate.units}" title="HASH RATE" align="right">${this.hashrate.rate.toFixed(2)}</flow-data-badge-graph>
		`;
