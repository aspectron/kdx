...
*/
	...
	async renderModuleInfo({ html }) {
		...
		let rate = html`
			<flow-data-field-graph style="min-width:128px;" sampler="${this.task.key}-block-rate" suffix=" / SEC" title="BLOCK RATE" align="right">${this.rate.toFixed(2)}</flow-data-field-graph>
			<flow-data-field-graph style="min-width:128px;" sampler="${this.task.key}-block-rate-avg" suffix=" / MIN" title="BLOCKS RATE (AVG)" align="right">${this.rate1m.toFixed(2)}</flow-data-field-graph>
		`;
