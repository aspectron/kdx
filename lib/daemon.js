...
module.exports = class Daemon  extends TermSink {
	...
	constructor(manager, task) {
		...
		this.on('running', () => {
			isNW && this['monitor-storage'] && this.createPoller(1 * 30 * 1000, async () => {
	...
	async updateStorageSize(render = true) {
		...
		if(this.storageUsageRateBytesPerHourAvg) {
...
//		let id = task.id;
		...
		const cpuSuffix = '%';
		const memorySuffix = '%';
		const cpu = this.procInfo?.cpu || 0;
		const memory = this.procInfo?.memory || 0;
		return html`
			...
			${
				this.manager.controller.advanced ? 
					html`<flow-data-badge class="uptime" has-colon title="UPTIME">${uptime}</flow-data-badge>` : ''
			}

			${
				this['monitor-storage'] ?
					html`
					<flow-data-badge-graph sampler="${this.task.key}-storage-used" has-colon suffix="${size_unit}" title="STORAGE USED" @click=${this.updateStorageSize}>${size}</flow-data-badge-graph>
					<flow-data-badge-graph sampler="${this.task.key}-storage-per-hour" has-colon x-prefix="5M AVG" suffix="${rate_hr_unit} / HOUR" title="STORAGE RATE | 5M AVG" @click=${this.updateStorageSize}>${rate_hr}</flow-data-badge-graph>
					<flow-data-badge-graph  sampler="${this.task.key}-storage-per-year" has-colon x-prefix="5M AVG" suffix="${rate_year_unit} / YEAR" title="STORAGE RATE | 5M AVG" @click=${this.updateStorageSize}>${rate_year}</flow-data-badge-graph>
					` : ''
			}

			${
				this.manager.controller.advanced && this.procInfo ? 
					html`
					<flow-data-badge-graph class="cpu" sampler="${this.task.key}-cpu" has-colon suffix="${cpuSuffix}" title="CPU">${cpu}</flow-data-badge-graph>
					<flow-data-badge-graph class="memory" sampler="${this.task.key}-memory" has-colon suffix="${memorySuffix}" title="MEMORY">${memory}</flow-data-badge-graph>
					` : ''
			}
