...
module.exports = class Daemon  extends TermSink {
		...
		this.on('start', ()=>{ 
			this.createPoller(1 * 60 * 1000, () => {
				return new Promise((resolve) => {
					...
					fs.stats(this.logFile, (err, stats) => {
						if(err) {
							console.log(err);
							return resolve();
						}
	...
	async renderModuleInfo(html) {
		...
		return html`
			<div class="task" ?running=${this.running} ?version=${version}>
				<div class="col-1"><span class="type">${task.type}</span><br/><span class="version">${version}</span></div>
	<div class="col-2"><span class="id">${prefix}<sub style="font-size:10px;">${suffix||''}</sub></span></div>
			</div>`;
	}
	...
	createPoller(interval, fn) {
		...
		const poller = async () => {
			console.log("POLLER",uid,"STARTING...");
			fn().then(()=>{
				console.log("POLLER",uid,"FINISHED...");
				if(this.pollers[uid])
					this.pollers[uid] = dpc(interval, poller);
			}).catch(()=>{
				console.log("POLLER",uid,"ABORTED...");
			})
