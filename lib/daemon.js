			...
			${	
				this.manager.controller.advanced ?
					Object.entries(pi).map(([k,v]) => {
						return html`<flow-data-badge-graph class="${k}" sampler="${this.task.key}-${k}" has-colon suffix="${v.suffix}" title="${v.title}">${v.value}</flow-data-badge-graph>`;
					})
				: ''
			}
	...
	async getVersion() {
		...
		return new Promise((resolve) => {
			this.exec(...this.vsargs_.args).then((text)=>{
				...
				let v = version.match(/(?<major>\d+)\.(?<minor>\d+)\.(?<patch>[\w-]+)/);
				if(v) {
