		...
		return html`
			...
			${
				this.manager.controller.advanced ? 
					html`<flow-data-field has-colon title="UPTIME">${uptime}</flow-data-field>` : ''
			}

			${
				this.task.conf['monitor-storage'] ?
					html`
					<flow-data-field-graph sampler="${this.task.key}-storage-used" has-colon suffix="${size_unit}" title="STORAGE USED" @click=${this.updateStorageSize}>${size}</flow-data-field-graph>
					<flow-data-field-graph sampler="${this.task.key}-storage-per-hour" has-colon x-prefix="5M AVG" suffix="${rate_hr_unit} / HOUR" title="STORAGE RATE | 5M AVG" @click=${this.updateStorageSize}>${rate_hr}</flow-data-field-graph>
					<flow-data-field-graph  sampler="${this.task.key}-storage-per-year" has-colon x-prefix="5M AVG" suffix="${rate_year_unit} / YEAR" title="STORAGE RATE | 5M AVG" @click=${this.updateStorageSize}>${rate_year}</flow-data-field-graph>
					` : ''
			}
			`;
	}
