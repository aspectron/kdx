...
module.exports = class Daemon  extends TermSink {
	...
	renderTab(html, T) {
		...
		return html`
			<div style="display:flex;flex-direction:row;${this.running?'':'color:red;'}">
				<div style="font-size:18px;"><flow-i18n text="${task.type}"></flow-i18n></div>
				<div style="font-size:10px; margin-top:8px;"><flow-i18n text="${task.id}"></flow-i18n></div>
			</div>`;
	}
	...
	async renderModuleInfo(html) {
		...
		let [size,unit] = this.storageSizeInBytes == -1 ? ['---',''] : this.storageSizeInBytes.toFileSize().split(' ');
		return html`
			<div class="task" ?running=${this.running} ?version=${version}>
				<div class="col-1">
					<span class="type">${task.type.toUpperCase()} ${prefix}<sub class="suffix">${suffix||''}</sub></span>
					<br/><span class="version">${version}</span>
				</div>
				<div class="col-2"><span class="id"></span></div>
			</div>
			
			<flow-data-field has-colon suffix="${unit}" title="STORAGE USED" @click=${this.updateStorageSize}>${size}</flow-data-field>
