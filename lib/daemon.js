...
//		let id = task.id;
		...
		const storageUsageRateBytesPerHour = this.storageUsageRateBytesPerHour.reduce((a,b) => a+b, 0) / this.storageUsageRateBytesPerHour.length;
		...
		return html`
			...
			<flow-data-field-graph has-colon suffix="${size_unit}" title="STORAGE USED" @click=${this.updateStorageSize}>${size}</flow-data-field-graph>
			<flow-data-field-graph has-colon x-prefix="5M AVG" suffix="${rate_hr_unit} / HOUR" title="STORAGE RATE | 5M AVG" @click=${this.updateStorageSize}>${rate_hr}</flow-data-field-graph>
			<flow-data-field-graph has-colon x-prefix="5M AVG" suffix="${rate_year_unit} / YEAR" title="STORAGE RATE | 5M AVG" @click=${this.updateStorageSize}>${rate_year}</flow-data-field-graph>
