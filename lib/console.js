...
class Console {
    ...
    constructor(controller, terminal) {
        ...
        ['help','wallet','rpc'].forEach(cmd=>this.handlers[cmd]=this[cmd].bind(this));
...
*/
    ...
    async wallet(...args) {
        ...
        switch(cmd) {
            ...
            case 'address':
            case 'show-address': {
                    return this.spawn(walletBin,['show-address', ...flags]);
            } break;
...
help(term) {
    switch(term) {
        default: {
            ...
            //migrate - upgrades Kasparov database schema\r
            this.terminal.write(`
wallet <command> - run native Kaspa CLI wallet command, where command is:\r
\r
    create\t\t\t create wallet\r
    send <address> <amount>\t send funds\r
    address\t\t\t alias for 'show-address'\r
    show-address\t\t show current wallet address\r
