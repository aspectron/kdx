...
class Console {
    ...
    getKasparovConfig() {
        return this.manager.tasks.filter(t=>t.type=="kasparovd").shift();
    }

    getKasparovAddress() {
        let cfg = this.getKasparovConfig();
        if(!cfg)
...
build - rebuild kaspa node\r
    ...
    async stop(ident) {
        if(ident) {
            ...
            this.terminal.write(`${jobs.length} service${jobs.length == 1 ? '' : 's'} stopped`);
            return Promise.resolve();
        } 
        else
    ...
    async start(ident) {
        if(ident) {
            ...
            await Promise.all(jobs);
            this.terminal.write(`${jobs.length} service${jobs.length == 1 ? '' : 's'} started`);
            return Promise.resolve();
        }
        else
