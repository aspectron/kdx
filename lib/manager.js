...
*/
	...
	registerString(owner, valueType, ident, value) {
		try {
			const [type, id] = owner.task.ident.split();
			if(valueType == 'port') {
				this.strings[`${type}-PORT`] = value;
				if(ident.toLowerCase() != type.toLowerCase())
					this.strings[`${type}-${ident}-PORT`] = value;
				this.strings[`${id}-PORT`] = value;
			} else if(valueType == 'address') {
				// 
				const port = parseInt(value.split(':').pop());
				if(!isNaN(port)) {
					this.strings[`${type}-PORT`] = port;
					if(ident.toLowerCase() != type.toLowerCase())
						this.strings[`${type}-${ident}-PORT`] = port;
					this.strings[`${id}-PORT`] = port;
				}
